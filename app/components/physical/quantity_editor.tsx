...
import { InputCategory } from "../NumberInput/InputCategory";
import { NumberKeyboard } from "../NumberInput/NumberKeyboard";
...
export function NumberInput({ product, onOk, initial, editExpiration }: Props) {
  ...
  return (
    <div className="grid place-items-center">
      <div className="border-b text-2xl border-black mb-2">{product}</div>
      {editExpiration ? (
        <DatePicker date={date} setDate={setDate} />
      ) : (
        <div className="mb-4">
          {initial.expiration_date
            ? format(initial.expiration_date, "PPP")
            : "No Expiration"}
        </div>
      )}
