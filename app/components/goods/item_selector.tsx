    ...
    return (<>
        <div className="flex gap-2 flex-wrap justify-center">
            <Button size="icon" variant="outline" className="shadow-md" onClick={onClick}><ArrowLeft /></Button>
            {
                categories.map((ele, index) => <div
                    className={`${selCategory == ele ? 'bg-foreground text-background ' : 'shadow-md'} border rounded-lg py-0.5 px-2`}
        ...
        <ScrollArea className="h-[calc(100dvh-200px)] border p-4 mt-4">
            <div className="flex flex-col gap-2">
                {
                    dexieGoods && dexieGoods.map((ele) => <div
                        className="border rounded-md p-2 border-b-2"
                        key={ele.id}
