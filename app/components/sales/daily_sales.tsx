...
export default function DailySales({ sales_arr, setIndex, deleteIndex, addItem }: Props) {
    return (
        <div className="grid grid-cols-[1fr_80px_100px_40px]  w-full">
            <div className="text-primary/80 bg-primary-foreground p-2 border-b">Item</div>
                ...
                return sale.items.map((item, index) => {
                    return <div key={index} className="contents">
                        <Link to={`/inventory/${item.id}`}>
                            <div className="bg-primary-foreground/20 pl-4 border-b-2 border-dashed py-2">{item.name}</div></Link>
                        ...
                        <div className={`${superindex % 2 == 0 ? "bg-foreground/5" : ""} grid place-items-center`}>
                            {index == 0 &&
                                <div>
                                    <Button variant="ghost" size="icon" onClick={() => setIndex(sale)}><Pencil /></Button>
            ...
            <div className="bg-primary-foreground p-2">
                <Button
                    variant="outline" className="shadow-md"
                    onClick={() => addItem()}>Add <Plus /></Button>
            </div>
        </div>
